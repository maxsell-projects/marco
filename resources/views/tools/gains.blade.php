...
@section('title', __('tools.gains.meta.title') . ' | Porthouse Private Office')
...
<div class="bg-brand-primary text-white pt-32 pb-20 text-center relative overflow-hidden">
    ...
    <div class="container mx-auto px-6 relative z-10" data-aos="fade-up">
        <p class="text-brand-premium font-mono text-xs uppercase tracking-[0.4em] mb-6">
            {{ __('tools.gains.hero.category') }}
        </p>
        <h1 class="text-4xl md:text-6xl font-didot leading-tight">
            {{ __('tools.gains.hero.title') }}
        </h1>
        <p class="text-gray-300 font-light max-w-2xl mx-auto mt-6 text-lg">
            {{ __('tools.gains.hero.description') }}
        </p>
    </div>
</div>

{{-- Configuração do Alpine com traduções injetadas --}}
<section class="py-20 bg-brand-background" 
         x-data="gainsForm({
             error_values: '{{ __('tools.gains.js.error_values') }}',
             error_contact: '{{ __('tools.gains.js.error_contact') }}',
             error_generic: '{{ __('tools.gains.js.error_generic') }}'
         })">
            ...
            <div class="lg:col-span-8 space-y-8">
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm border-t-4 border-brand-premium" data-aos="fade-up">
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <h3 class="text-xl font-didot text-brand-primary">{{ __('tools.gains.form.step1_title') }}</h3>
                    </div>
                    
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">{{ __('tools.gains.form.buy_value') }} (€)</label>
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">{{ __('tools.gains.form.year') }}</label>
                                <select x-model="form.acquisition_year" class="w-full bg-gray-50 border-0 border-b-2 border-gray-200 px-4 py-3 focus:ring-0 focus:border-brand-premium cursor-pointer text-brand-primary">
                                    @foreach(range(date('Y'), 1901) as $year)
                                        <option value="{{ $year }}">{{ $year }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                                <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">{{ __('tools.gains.form.month') }}</label>
                                <select x-model="form.acquisition_month" class="w-full bg-gray-50 border-0 border-b-2 border-gray-200 px-4 py-3 focus:ring-0 focus:border-brand-premium cursor-pointer text-brand-primary">
                                    @foreach(['Janeiro','Fevereiro','Março','Abril','Maio','Junho','Julho','Agosto','Setembro','Outubro','Novembro','Dezembro'] as $index => $month)
                                        {{-- Enviamos o valor em PT para o backend (se necessário), mas mostramos traduzido --}}
                                        <option value="{{ $month }}">{{ __('tools.gains.months.' . ($index + 1)) }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-100">
                        <label class="block text-sm font-medium text-gray-700 mb-3">{{ __('tools.gains.form.self_built_q') }}</label>
                        <div class="flex gap-6">
                            <label class="inline-flex items-center cursor-pointer group">
                                ...
                                <span class="ml-2 text-sm text-gray-600 group-hover:text-brand-primary transition-colors">{{ __('tools.gains.common.yes') }}</span>
                            </label>
                            <label class="inline-flex items-center cursor-pointer group">
                                <input type="radio" value="Não" x-model="form.self_built" class="peer sr-only">
                                <span class="w-4 h-4 border border-gray-300 rounded-full peer-checked:bg-brand-premium peer-checked:border-brand-premium transition-all"></span>
                                <span class="ml-2 text-sm text-gray-600 group-hover:text-brand-primary transition-colors">{{ __('tools.gains.common.no') }}</span>
                            </label>
                        </div>
                        <p class="text-[10px] text-gray-400 mt-2 italic">*{{ __('tools.gains.form.self_built_note') }}</p>
                    </div>
                </div>
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm border-t-4 border-brand-premium" data-aos="fade-up" data-aos-delay="100">
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <h3 class="text-xl font-didot text-brand-primary">{{ __('tools.gains.form.step2_title') }}</h3>
                    </div>

                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-400 mb-2">{{ __('tools.gains.form.sell_value') }} (€)</label>
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm border-t-4 border-brand-premium" data-aos="fade-up" data-aos-delay="200">
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <h3 class="text-xl font-didot text-brand-primary">{{ __('tools.gains.form.step3_title') }}</h3>
                    </div>

                    <div class="mb-6">
                        <label class="block text-sm font-medium text-gray-700 mb-3">{{ __('tools.gains.form.expenses_q') }}</label>
                        <div class="flex gap-6">
                            <label class="inline-flex items-center cursor-pointer group">
                                <input type="radio" value="Sim" x-model="form.has_expenses" class="peer sr-only">
                                <div class="px-4 py-2 border border-gray-200 rounded text-sm peer-checked:bg-brand-primary peer-checked:text-white peer-checked:border-brand-primary transition-all group-hover:border-brand-primary">{{ __('tools.gains.common.yes') }}</div>
                            </label>
                            <label class="inline-flex items-center cursor-pointer group">
                                <input type="radio" value="Não" x-model="form.has_expenses" class="peer sr-only">
                                <div class="px-4 py-2 border border-gray-200 rounded text-sm peer-checked:bg-brand-primary peer-checked:text-white peer-checked:border-brand-primary transition-all group-hover:border-brand-primary">{{ __('tools.gains.common.no') }}</div>
                            </label>
                        </div>
                    </div>
                    ...
                    <div x-show="form.has_expenses === 'Sim'" x-transition class="grid grid-cols-1 md:grid-cols-2 gap-6 bg-[#F5F7FA] p-6 rounded border border-gray-200">
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">{{ __('tools.gains.form.exp_works') }} (€)</label>
                            ...
                            <p class="text-[10px] text-gray-400 mt-1">*{{ __('tools.gains.form.exp_works_note') }}</p>
                        </div>
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">{{ __('tools.gains.form.exp_imt') }} (€)</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">{{ __('tools.gains.form.exp_comm') }} (€)</label>
                        ...
                        <div>
                            <label class="block text-[10px] font-bold uppercase tracking-widest text-gray-500 mb-2">{{ __('tools.gains.form.exp_other') }} (€)</label>
                ...
                <div class="bg-white p-8 rounded-lg shadow-sm border-t-4 border-brand-premium" data-aos="fade-up" data-aos-delay="300">
                    <div class="flex items-center gap-4 mb-8">
                        ...
                        <h3 class="text-xl font-didot text-brand-primary">{{ __('tools.gains.form.step4_title') }}</h3>
                    </div>

                    {{-- Venda ao Estado --}}
                    <div class="mb-8 border-b border-gray-100 pb-6">
                        <label class="block text-sm font-bold text-gray-700 mb-3">{{ __('tools.gains.form.state_sold_q') }}</label>
                        <div class="flex gap-6">
                            <label class="inline-flex items-center cursor-pointer group">
                                <input type="radio" value="Sim" x-model="form.sold_to_state" @change="resetHPPFields" class="peer sr-only">
                                <span class="w-4 h-4 border border-gray-300 rounded-full peer-checked:bg-brand-premium peer-checked:border-brand-premium transition-all"></span>
                                <span class="ml-2 text-sm text-gray-600">{{ __('tools.gains.common.yes') }}</span>
                            </label>
                            <label class="inline-flex items-center cursor-pointer group">
                                <input type="radio" value="Não" x-model="form.sold_to_state" @change="resetHPPFields" class="peer sr-only">
                                <span class="w-4 h-4 border border-gray-300 rounded-full peer-checked:bg-brand-premium peer-checked:border-brand-premium transition-all"></span>
                                <span class="ml-2 text-sm text-gray-600">{{ __('tools.gains.common.no') }}</span>
                            </label>
                        </div>
                        <div x-show="form.sold_to_state === 'Sim'" x-transition class="mt-4 p-3 bg-blue-50 border border-blue-100 text-blue-800 text-xs font-bold rounded">
                            ℹ️ {{ __('tools.gains.form.state_sold_info') }}
                        </div>
                    </div>
                    ...
                    <div x-show="form.sold_to_state === 'Não'" x-transition class="space-y-8">
                        ...
                        {{-- HPP --}}
                        <div>
                            <label class="block text-sm font-bold text-gray-700 mb-3">{{ __('tools.gains.form.hpp_label') }}</label>
                            <div class="space-y-3">
                                <label class="flex items-center cursor-pointer p-3 border border-gray-200 rounded hover:bg-gray-50 transition">
                                    ...
                                    <span class="ml-3 text-sm text-gray-700">{{ __('tools.gains.form.hpp_yes') }}</span>
                                </label>
                                <label class="flex items-center cursor-pointer p-3 border border-gray-200 rounded hover:bg-gray-50 transition">
                                    ...
                                    <span class="ml-3 text-sm text-gray-700">{{ __('tools.gains.form.hpp_under12') }}</span>
                                </label>
                                <label class="flex items-center cursor-pointer p-3 border border-gray-200 rounded hover:bg-gray-50 transition">
                                    <input type="radio" value="Não" x-model="form.hpp_status" @change="resetReinvestmentFields" class="accent-brand-premium w-4 h-4">
                                    <span class="ml-3 text-sm text-gray-700">{{ __('tools.gains.form.hpp_no') }}</span>
                                </label>
                            </div>
                        </div>
                        ...
                        <div class="bg-[#F5F7FA] p-6 rounded border border-gray-200">
                            <h4 class="text-sm font-bold text-brand-primary uppercase tracking-widest mb-4">{{ __('tools.gains.form.reinvest_section') }}</h4>
                            
                            {{-- Reinvestimento --}}
                            <div class="mb-6" x-show="form.hpp_status === 'Sim'">
                                <label class="block text-xs font-bold text-gray-500 mb-2">{{ __('tools.gains.form.reinvest_q') }}</label>
                                <div class="flex gap-4 mb-3">
                                    <label class="inline-flex items-center cursor-pointer"><input type="radio" value="Sim" x-model="form.reinvest_intention" class="accent-brand-premium w-4 h-4"><span class="ml-2 text-sm">{{ __('tools.gains.common.yes') }}</span></label>
                                    <label class="inline-flex items-center cursor-pointer"><input type="radio" value="Não" x-model="form.reinvest_intention" class="accent-brand-premium w-4 h-4"><span class="ml-2 text-sm">{{ __('tools.gains.common.no') }}</span></label>
                                </div>
                                <div x-show="form.reinvest_intention === 'Sim'" x-transition>
                                    <input type="number" step="0.01" x-model="form.reinvestment_amount" class="w-full border-gray-300 rounded text-sm focus:border-brand-cta focus:ring-0" placeholder="{{ __('tools.gains.form.reinvest_placeholder') }}">
                                </div>
                            </div>

                            {{-- Amortização --}}
                            <div class="mb-6">
                                <label class="block text-xs font-bold text-gray-500 mb-2">{{ __('tools.gains.form.amortization_q') }}</label>
                                <div class="flex gap-4 mb-3">
                                    <label class="inline-flex items-center cursor-pointer"><input type="radio" value="Sim" x-model="form.amortize_credit" class="accent-brand-premium w-4 h-4"><span class="ml-2 text-sm">{{ __('tools.gains.common.yes') }}</span></label>
                                    <label class="inline-flex items-center cursor-pointer"><input type="radio" value="Não" x-model="form.amortize_credit" class="accent-brand-premium w-4 h-4"><span class="ml-2 text-sm">{{ __('tools.gains.common.no') }}</span></label>
                                </div>
                                <div x-show="form.amortize_credit === 'Sim'" x-transition>
                                    <input type="number" step="0.01" x-model="form.amortization_amount" class="w-full border-gray-300 rounded text-sm focus:border-brand-cta focus:ring-0" placeholder="{{ __('tools.gains.form.reinvest_placeholder') }}">
                                </div>
                            </div>

                            {{-- Rendimentos --}}
                            <div>
                                <label class="block text-xs font-bold text-gray-500 mb-2">{{ __('tools.gains.form.income_label') }}</label>
            ...
            <div class="lg:col-span-4" id="results-area">
                <div class="sticky top-32 space-y-6">
                    ...
                    <div x-show="!hasCalculated" class="bg-white border-2 border-dashed border-gray-200 rounded-lg p-10 text-center text-gray-400">
                        ...
                        <p class="text-xs uppercase tracking-widest">{{ __('tools.gains.results.waiting') }}</p>
                    </div>
                    ...
                    <div x-show="hasCalculated" x-transition style="display: none;" class="space-y-6">
                        ...
                        <div class="bg-brand-primary text-white rounded-lg p-8 shadow-2xl relative overflow-hidden">
                            ...
                            <h3 class="text-xs font-bold text-brand-premium uppercase tracking-[0.2em] mb-2">{{ __('tools.gains.results.estimated_tax') }}</h3>
                            ...
                            <div class="space-y-3 text-sm font-light border-t border-white/10 pt-4">
                                <div class="flex justify-between">
                                    <span class="text-gray-400">{{ __('tools.gains.results.gross_gain') }}</span>
                                    <span x-text="results.gross_gain_fmt + ' €'"></span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-400">{{ __('tools.gains.results.taxable_gain') }}</span>
                        ...
                        <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden text-sm">
                            <div class="bg-gray-50 px-6 py-4 border-b border-gray-200 font-bold text-brand-primary uppercase text-xs tracking-widest">
                                {{ __('tools.gains.results.details_title') }}
                            </div>
                            <div class="p-6 space-y-3">
                                <div class="flex justify-between">
                                    <span class="text-gray-500">{{ __('tools.gains.results.sale') }}</span>
                                    <span class="font-medium" x-text="results.sale_fmt + ' €'"></span>
                                </div>
                                <div class="flex justify-between">
                                    <span class="text-gray-500">{{ __('tools.gains.results.coef') }}</span>
                                    <span class="font-medium" x-text="results.coefficient"></span>
                                </div>
                                <div class="flex justify-between text-brand-cta">
                                    <span>{{ __('tools.gains.results.acq_updated') }}</span>
                                    <span x-text="'- ' + results.acquisition_updated_fmt + ' €'"></span>
                                </div>
                                <div class="flex justify-between text-brand-cta">
                                    <span>{{ __('tools.gains.results.expenses') }}</span>
                                ...
                                <div class="flex justify-between text-brand-secondary font-bold" x-show="results.reinvestment_fmt !== '0,00' || form.amortize_credit === 'Sim'">
                                    <span>{{ __('tools.gains.results.exemption') }}</span>
                        ...
                        <div class="p-4 bg-brand-premium/10 border border-brand-premium/30 rounded text-xs text-brand-primary leading-relaxed">
                            <strong class="block mb-1 font-bold">{{ __('tools.gains.results.note') }}:</strong>
                            {{ __('tools.gains.results.note_text') }}
                        </div>
            ...
            <div x-show="showLeadModal" x-transition.scale class="relative inline-block bg-white rounded-lg text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:max-w-lg w-full border-t-4 border-brand-premium">
                <div class="bg-white px-8 py-10">
                    <div class="text-center mb-8">
                        <h3 class="text-2xl font-didot text-brand-primary mb-2">{{ __('tools.gains.modal.title') }}</h3>
                        <p class="text-sm text-gray-500 font-light">{{ __('tools.gains.modal.desc') }}</p>
                    </div>
                    
                    <div class="space-y-6">
                        <div>
                            <label class="block text-[10px] uppercase tracking-widest text-gray-400 mb-2">{{ __('tools.gains.modal.name') }}</label>
                        ...
                        <div>
                            <label class="block text-[10px] uppercase tracking-widest text-gray-400 mb-2">{{ __('tools.gains.modal.email') }}</label>
...
<script>
    function gainsForm(translations) {
        return {
            hasCalculated: false,
            showLeadModal: false,
            ...
            openModal() {
                if(!this.form.acquisition_value || !this.form.sale_value) {
                    alert(this.translations.error_values);
                    return;
                }
            ...
            async submit() {
                if(!this.form.lead_name || !this.form.lead_email) {
                    alert(this.translations.error_contact);
                    return;
                }
                ...
                try {
                    ...
                    if (!response.ok) {
                        alert(this.translations.error_generic);
                        return;
                    }
