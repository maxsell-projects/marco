...
<div class="flex justify-between items-center mb-8">
    <div>
        <h2 class="text-3xl font-didot text-brand-primary">Novo Imóvel</h2>
        <p class="text-xs text-gray-500 mt-1">Preencha os detalhes do novo ativo do portfólio.</p>
    </div>
...
<div class="max-w-5xl mx-auto pb-20">
    ...
    <form action="{{ route('admin.properties.store') }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="bg-white p-8 rounded-lg shadow-sm border border-gray-100 relative overflow-hidden">
            <div class="absolute top-0 right-0 p-8">
                ...
                <div class="flex flex-col items-end gap-2">
                    ...
                    <label class="flex items-center gap-2 cursor-pointer mt-2">
                        <input type="checkbox" name="is_featured" value="1" class="rounded border-gray-300 text-brand-premium focus:ring-brand-premium h-4 w-4">
                        <span class="text-[10px] text-gray-400 font-bold uppercase tracking-widest">Destacar na Home</span>
                    </label>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 gap-6 pr-0 md:pr-40"> {{-- Padding right para não bater no switch --}}
                <div>
                    ...
                    <input type="text" name="title" value="{{ old('title') }}" required 
                           ...
                           placeholder="Ex: Penthouse Duplex com Vista Rio">
                </div>
                ...
                <div>
                     ...
                     <div class="flex gap-6">
                         <label class="flex items-center gap-2 cursor-pointer">
                             <input type="radio" name="status" value="Venda" checked class="text-brand-cta focus:ring-brand-cta">
                             <span class="text-sm text-gray-600">Para Venda</span>
                         </label>
                         <label class="flex items-center gap-2 cursor-pointer">
                             <input type="radio" name="status" value="Arrendamento" class="text-brand-cta focus:ring-brand-cta">
        ...
        <div class="bg-white p-8 rounded-lg shadow-sm border border-gray-100">
            ...
            <div class="grid grid-cols-2 md:grid-cols-5 gap-6">
                <div>
                    ...
                    <input type="number" name="area_gross" value="{{ old('area_gross') }}" class="w-full border-gray-200 rounded px-4 py-3 focus:border-brand-cta focus:ring-0 text-sm">
                </div>
                <div>
                    <label class="block text-[10px] uppercase tracking-widest text-gray-400 mb-2 font-bold">Quartos</label>
                    <input type="number" name="bedrooms" value="{{ old('bedrooms') }}" class="w-full border-gray-200 rounded px-4 py-3 focus:border-brand-cta focus:ring-0 text-sm">
                </div>
                <div>
                    <label class="block text-[10px] uppercase tracking-widest text-gray-400 mb-2 font-bold">WC's</label>
                    <input type="number" name="bathrooms" value="{{ old('bathrooms') }}" class="w-full border-gray-200 rounded px-4 py-3 focus:border-brand-cta focus:ring-0 text-sm">
                </div>
                <div>
                    <label class="block text-[10px] uppercase tracking-widest text-gray-400 mb-2 font-bold">Garagem</label>
                    <input type="number" name="garages" value="{{ old('garages') }}" class="w-full border-gray-200 rounded px-4 py-3 focus:border-brand-cta focus:ring-0 text-sm">
                </div>
                <div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                @foreach([
                    ...
                    <label class="flex items-center gap-3 p-3 border border-gray-100 rounded cursor-pointer hover:bg-gray-50 hover:border-brand-premium/30 transition-all select-none group">
                        <input type="checkbox" name="{{ $field }}" class="accent-brand-premium w-4 h-4 rounded border-gray-300 focus:ring-brand-premium text-brand-premium" {{ old($field) ? 'checked' : '' }}> 
        ...
        <div class="bg-white p-8 rounded-lg shadow-sm border border-gray-100">
            <div class="flex items-center gap-3 mb-6 border-b border-gray-100 pb-4">
                ...
                <h3 class="text-lg font-serif text-brand-primary">Media</h3>
            </div>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 mb-6">
                <div class="p-6 border-2 border-dashed border-gray-200 rounded-lg hover:border-brand-premium transition-colors bg-gray-50/50">
                    <label class="block text-[10px] uppercase tracking-widest text-gray-500 mb-4 font-bold text-center">Foto de Capa</label>
                ...
                <div class="p-6 border-2 border-dashed border-gray-200 rounded-lg hover:border-brand-premium transition-colors bg-gray-50/50">
                    <label class="block text-[10px] uppercase tracking-widest text-gray-500 mb-4 font-bold text-center">Galeria Completa</label>
                    <input type="file" id="gallery-input" name="gallery[]" multiple accept="image/*" class="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-full file:border-0 file:text-xs file:font-bold file:bg-gray-200 file:text-gray-700 hover:file:bg-gray-300 cursor-pointer">
                </div>
            </div>
        ...
        <div class="bg-white p-8 rounded-lg shadow-sm border border-gray-100">
            ...
            <textarea name="description" rows="5" class="w-full border-gray-200 rounded px-4 py-3 focus:border-brand-cta focus:ring-0 text-sm leading-relaxed" placeholder="Descreva o imóvel...">{{ old('description') }}</textarea>
        </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        input.addEventListener('change', function() {
            for(let i = 0; i < this.files.length; i++) {
                ...
                div.className = "relative h-20 w-full rounded overflow-hidden border border-gray-200 shadow-sm group bg-white";
                ...
                btn.innerHTML = '&times;';
                btn.className = "absolute top-1 right-1 bg-red-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-[10px] shadow-md hover:bg-red-600 focus:outline-none z-10 opacity-0 group-hover:opacity-100 transition";
